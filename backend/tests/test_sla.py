"""Тест обязательного сценария №3: Защита от повторных алертов и переходы SLA.

Назначение:
- Верификация переходов `normal` -> `warning` (60с) -> `overdue` (180с).
- Проверка создания ровно 1 записи в Outbox для каждого порогового события.
- Подтверждение отсутствия повторных алертов при повторных циклах сканирования.
"""
