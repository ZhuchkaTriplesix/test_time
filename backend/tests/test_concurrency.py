"""Тест обязательного сценария №2: Неблокирующий Event Loop.

Назначение:
- Верификация поведения API при искусственной задержке в 2.0 секунды в адаптере нотификаций.
- Проверка мгновенного ответа `GET /health` и `GET /api/tickets` (< 200 мс) во время отправки алерта.
"""
