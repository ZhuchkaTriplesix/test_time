"""Тест обязательного сценария №1: Идемпотентность при конкуренции.

Назначение:
- Верификация отправки 20 одинаковых событий клиента параллельно (`asyncio.gather`).
- Проверка создания ровно 1 тикета и 1 записи в таблице `events`.
- Проверка HTTP-статусов: ровно 1 ответ `201 Created` и 19 ответов `200 OK`.
"""
