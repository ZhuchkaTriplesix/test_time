"""Тест обязательного сценария №5: Расчет аналитических метрик и медианы.

Назначение:
- Верификация поведения `GET /api/metrics` на пустой выборке (возвращает `null`/0 без ошибок).
- Верификация расчета метрик на фиксированном наборе данных, включая корректность непрерывной медианы.
"""
