"""Ядро асинхронного подключения к базе данных PostgreSQL.

Назначение:
- Инициализация и управление пулом соединений через `AsyncEngine` (драйвер `asyncpg`).
- Создание фабрики асинхронных сессий `async_sessionmaker`.
- Грациозное открытие и закрытие пула соединений при старте и завершении приложения.
"""
