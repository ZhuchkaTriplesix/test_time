"""Периодический фоновый сканер нарушений порогов SLA (SLA Sweeper).

Назначение:
- Регулярный поиск открытых обращений, превысивших пороги ожидания (60 сек - Warning, 180 сек - Overdue).
- Атомарный перевод статуса SLA тикета и генерация задачи в `outbox_events` в рамках одной транзакции.
- Защита от дублирующих алертов при повторных прогонах.
"""
